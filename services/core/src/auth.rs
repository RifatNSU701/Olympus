...
pub async fn require_auth(State(state): State<AppState>, mut request: Request<Body>, next: Next) -> Result<Response, StatusCode> {
    ...
    let data = decode::<Claims>(token, &DecodingKey::from_secret(state.jwt_secret.as_bytes()), &validation).map_err(|_| StatusCode::UNAUTHORIZED)?;
    request.extensions_mut().insert(data.claims);
    Ok(next.run(request).await)
}

pub async fn check_user(state: &AppState, user_id: Uuid) -> Result<(), sqlx::Error> {
    sqlx::query("SELECT 1 FROM users WHERE id = $1 AND status = 'ACTIVE'").bind(user_id).fetch_optional(&state.pool).await?.ok_or(sqlx::Error::RowNotFound).map(|_| ())
}
