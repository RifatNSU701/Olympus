...
async fn ensure_cart(state: &AppState, buyer_id: Uuid) -> Result<Uuid, StatusCode> {
    sqlx::query_scalar(
        "INSERT INTO carts (buyer_id) VALUES ($1) ON CONFLICT (buyer_id) DO UPDATE SET updated_at = now() RETURNING id",
    )
    .bind(buyer_id)
...
pub async fn remove(
    ...
    sqlx::query("DELETE FROM cart_items AS ci USING carts AS c WHERE ci.id = $1 AND ci.cart_id = c.id AND c.buyer_id = $2")
        .bind(item_id)
        .bind(claims.sub)
        .execute(&state.pool)
        .await
