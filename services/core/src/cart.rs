...
#[derive(Serialize)]
pub struct CartItemResponse { pub id: Uuid, pub product_id: Uuid, pub product_name: String, pub unit_price: Decimal, pub available_stock: i32, pub quantity: i32, pub line_total: Decimal }
#[derive(Serialize)]
pub struct CartResponse { pub id: Uuid, pub items: Vec<CartItemResponse>, pub subtotal: Decimal }
#[derive(Deserialize)]
pub struct AddItemRequest { pub product_id: Uuid, pub quantity: i32 }
#[derive(Deserialize)]
...
pub async fn get(State(state): State<AppState>, Extension(claims): Extension<Claims>) -> Result<Json<CartResponse>, StatusCode> {
    ...
    let items = rows.into_iter().map(|(id, product_id, name, price, stock, quantity)| { let line_total = price * Decimal::from(quantity); subtotal += line_total; CartItemResponse { id, product_id, product_name: name, unit_price: price, available_stock: stock, quantity, line_total } }).collect();
    Ok(Json(CartResponse { id: cart_id, items, subtotal }))
}

pub async fn add(State(state): State<AppState>, Extension(claims): Extension<Claims>, Json(req): Json<AddItemRequest>) -> Result<Json<CartResponse>, StatusCode> {
    if req.quantity <= 0 { return Err(StatusCode::BAD_REQUEST); }
    ...
    let stock: Option<i32> = sqlx::query_scalar("SELECT stock FROM products WHERE id = $1 AND status = 'ACTIVE' FOR UPDATE").bind(req.product_id).fetch_optional(&mut *tx).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let stock = stock.ok_or(StatusCode::NOT_FOUND)?;
    let existing: Option<i32> = sqlx::query_scalar("SELECT quantity FROM cart_items WHERE cart_id = $1 AND product_id = $2 FOR UPDATE").bind(cart_id).bind(req.product_id).fetch_optional(&mut *tx).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
