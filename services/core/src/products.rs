...
#[derive(Deserialize)] pub struct ListQuery { pub page: Option<i64>, pub limit: Option<i64>, pub category_id: Option<Uuid> }
#[derive(Deserialize)] pub struct CreateProduct { pub category_id: Option<Uuid>, pub name: String, pub slug: String, pub description: Option<String>, pub price: Decimal, pub stock: i32 }
...
pub async fn list(State(state): State<AppState>, Query(q): Query<ListQuery>) -> Result<Json<Vec<Product>>, StatusCode> {
    ...
    let rows = sqlx::query_as::<_, (Uuid, Uuid, Option<Uuid>, String, String, Option<String>, Decimal, i32, String)>(
    ...
    ).bind(q.category_id).bind(limit).bind(offset).fetch_all(&state.pool).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn get(State(state): State<AppState>, Path(id): Path<Uuid>) -> Result<Json<Product>, StatusCode> {
    let r = sqlx::query_as::<_, (Uuid, Uuid, Option<Uuid>, String, String, Option<String>, Decimal, i32, String)>("SELECT id, seller_id, category_id, name, slug, description, price, stock, status FROM products WHERE id = $1 AND status = 'ACTIVE'").bind(id).fetch_optional(&state.pool).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?.ok_or(StatusCode::NOT_FOUND)?;
...
pub async fn create(State(state): State<AppState>, Extension(claims): Extension<Claims>, Json(req): Json<CreateProduct>) -> Result<(StatusCode, Json<Product>), StatusCode> {
    if claims.role != "SELLER" && claims.role != "ADMIN" && claims.role != "SUPER_ADMIN" { return Err(StatusCode::FORBIDDEN); }
    ...
    let r = sqlx::query_as::<_, (Uuid, Uuid, Option<Uuid>, String, String, Option<String>, Decimal, i32, String)>("INSERT INTO products (seller_id, category_id, name, slug, description, price, stock, status) VALUES ($1,$2,$3,$4,$5,$6,$7,'ACTIVE') RETURNING id, seller_id, category_id, name, slug, description, price, stock, status").bind(claims.sub).bind(req.category_id).bind(req.name.trim()).bind(&slug).bind(req.description).bind(req.price).bind(req.stock).fetch_one(&state.pool).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
